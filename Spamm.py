...
def logo():
 ...
 | |/ / | | |   \  |    | |    | |  \  |%sAuthor by %sdwiirfan187%s

 | |    | | | )  | | __ | |____| |   \ |%sGithub %sgithub.com/sdwiirfan187%s

 |_|    |_| |___/  |/  \| |    | |    \|%sTeam %s%sDwiGroup
...
def input():
  ...
  nom = raw_input("%s[%s?%s] %sMasukkan nomor target cok awali (815,838,851,xxx) : "%(pu,me,pu,pu))

  if len(nom) < 5:

    print "%s[%s!%s] %sMasukkan nomor target dengan benar cok!!"%(pu,me,pu,me)

    input()

  elif nom.startswith(tuple(["62","+62","0"])):

    print "%s[%s!%s] %sMasukkan nomor tanpa 62, +62, ataupun 0 ngerti ra su\n%s[%s!%s] %sContoh : 85877162199"%(pu,me,pu,ku,pu,me,pu,ku)
    ...
    jum = int(raw_input("%s[%s?%s] %sMasukkan jumlah spam "Ojo Akeh Akeh Su mesakne loe: "%(pu,me,pu,pu)))
